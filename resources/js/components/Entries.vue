<template>
    <div>
        <div class="bg-slate-200 p-2 rounded-md sm:h-[100px] border border-slate-200 hover:border-2 hover:border-cyan-700 transition duration-500 flex justify-center items-center hover:shadow-2xl hover:-translate-y-2 hover:bg-slate-100" :class="!isLastEntry? 'mb-5':''">
            <div class="w-full sm:grid xl:grid-cols-7 lg:grid-cols-5 sm:grid-cols-4 gap-0 space-y-3 sm:space-y-0 h-[80%]">
                <EntrySegment :value="entry.id" :idField="true" />
...
import EntrySegment from './EntrySegment.vue'
...
export default {
    ...
    created() {
        if(this.entry.id == this.allEntries[0][this.allEntries[0].length-1].id) {
