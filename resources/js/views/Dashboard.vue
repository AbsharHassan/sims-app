                    ...
                    <div class="sm:flex items-baseline justify-between ">
                        ...
                        <div class="mb-5 sm:mb-0">
                            <router-link :to="{name: 'CreateEntry'}">
                                <Button text="Create New Entry" class="text-sm rounded-md px-2 py-[5px] grow-0 bg-sky-700 hover:bg-black mb-3 w-full">
...
export default {
    ...
    computed: {
        ...mapGetters(['allEntries', 'loadingStatus', 'isEmptyRespone', 'navWidth']),
        entriesArray() {
            return this.$store.getters.allEntries[this.$store.getters.allEntries.length - 1]
        }
    },
    async created() {
