        ...
        <div>
                    ...
                    <section class="mt-8">
                        <form class="flex flex-col" @submit="onSubmit">
                            <div class="mb-6 pt-3 rounded bg-slate-200/75">
                                <BasicAuthenticationInput v-model="formFields.email" name="Email" type="email" :class="allErrors==401? 'border-red-600' : ''" objKey="email"/>
                            </div>
                            <div class="mb-6 pt-3 rounded bg-slate-200/75">
                                <BasicAuthenticationInput v-model="formFields.password" name="Password" :type="showPassword? 'text': 'password'" @togglePassword="togglePassword()" :class="allErrors==401? 'border-red-600' : ''" objKey="password">
                                    <i v-if="showPassword" class="bi bi-eye-slash-fill"></i>
                                    <i v-else class="bi bi-eye-fill"></i>
                                </BasicAuthenticationInput>
                            </div>
                            <div class="sm:h-[25px]">
                                <div class="text-center sm:text-left sm:flex justify-between items-baseline">
                                    <div class="font-bold text-red-600 badCreds">
                            ...
                            <Button :text="!loadingStatus? 'Sign In':''" :processingPresent="true" class="mt-5 h-[50px]" @submit="onSubmit">
            ...
            <section>
                <div class="max-w-lg mx-auto text-center mt-[-8px] mb-6">
                    <p class="text-sky-800">Don't have an account? <router-link :to="{name: 'Register'}" class="font-bold hover:underline">Sign up here</router-link>.</p>
                </div>
            </section>
        </div>
    </div>
