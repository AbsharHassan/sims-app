    ...
    <div>
        ...
        <div class="h-full md:grid md:grid-rows-[60px_minmax(60px,_1fr)]" :class="`xl:grid-cols-[${navWidth}px_minmax(${navWidth}px,_1fr)]`">
            ...
            <transition name="courseEntries" appear>    
                <div class="shadow-md m-5 px-5 pt-1 pb-5 hover:shadow-2xl transition duration-500 border-t-8 border-t-sky-500/20">
                    <div class="sm:flex items-baseline justify-between ">
                        <div class="text-slate-600 text-lg font-bold flex mt-4 justify-center items-center mb-5 sm:mb-0">
                            Click to view full details
                        </div>
                        <div class="mb-5 sm:mb-0">
                            <router-link :to="{name: 'CreateCourse'}">
                                <Button text="Create New Course" class="text-sm rounded-md px-2 py-[5px] grow-0 bg-sky-700 hover:bg-black mb-3 w-full">
                    ...
                    <div v-else class="sm:flex flex-wrap justify-center">
                        <div v-for="course in coursesArray" :key="course.course_id" >
                            <div>
                                <router-link :to="{name: 'SingleCourse', params: {course_id: course.course_id}}">
...
export default {
    ...
    computed: {
        ...mapGetters(['allCourses', 'loadingStatus', 'isEmptyRespone', 'navWidth']),
        coursesArray() {
            return this.$store.getters.allCourses[this.$store.getters.allCourses.length - 1]
        }
    },
    async created() {
