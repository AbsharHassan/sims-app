...
const actions = {
        ...
        await axios.post('/api/register', formFields)
                    .then(response => {
                        if(response.data.token) {
                            ...
                            sessionStorage.setItem('CURRENT-USER-ID', response.data.user.id)
                            sessionStorage.setItem('CURRENT-USER-EMAIL', response.data.user.email)
                            sessionStorage.setItem('CURRENT-USER-NAME', response.data.user.name)
    ...
    async editCurrentUser({ commit }, formFields) {
        commit('newLoadingStatus', true)
        var errors = {}
        await axios.put(`/api/users/${formFields.id}`, formFields)
        .then(response => {
            console.log(response)
            commit('setFlashMessage', response.data.message)
            sessionStorage.setItem('CURRENT-USER-ID', response.data.user.id)
    ...
    async changePassword({ commit }, formFields) {
        commit('newLoadingStatus', true)
        var errors = {}
        await axios.put(`/api/users/${formFields.id}/change-password`, formFields)
        .then(response => {
            console.log(response) 
            commit('setFlashMessage', response.data.message)
        }).catch(error => {
    ...
    async getFamilyCreatePage({ commit }, id) {
        ...
        .then(response => {
            if(response.data.canProceed) {
                router.push({name: 'FamilyDataEntry', params: {student_id: id}})
            } else {
        ...
        .then(response => {
            response.data.forEach(entry => {
                ...
                if(todayDate.getMonth() > birthDate.getMonth()) {
                    entry.age = todayDate.getFullYear() - birthDate.getFullYear()
                } else if(todayDate.getMonth() == birthDate.getMonth()) {
                    if(todayDate.getDate() >= birthDate.getDate()) {
                        entry.age = todayDate.getFullYear() - birthDate.getFullYear()
                    } else {
                        entry.age = todayDate.getFullYear() - birthDate.getFullYear() - 1
                    }
                } else {
                    entry.age = todayDate.getFullYear() - birthDate.getFullYear() - 1;
                }
    ...
    async getSingleEntry({ commit }, id) {
        ...
        .then(response => {
            if(response.data) {
                ...
                if(todayDate.getMonth() > birthDate.getMonth()) {
                    response.data.student.age = todayDate.getFullYear() - birthDate.getFullYear()
                } else if(todayDate.getMonth() == birthDate.getMonth()) {
                    if(todayDate.getDate() >= birthDate.getDate()) {
                        response.data.student.age = todayDate.getFullYear() - birthDate.getFullYear()
                    } else {
                        response.data.student.age = todayDate.getFullYear() - birthDate.getFullYear() - 1
                    }
                } else {
                    response.data.student.age = todayDate.getFullYear() - birthDate.getFullYear() - 1;
                }
                response.data.academics.latest_cgpa = response.data.academics[`${response.data.student.current_sem - 1}`].cgpa
    ...
    async addEntryStudent({ commit }, formFields) {
        var errors = {};
        commit('newLoadingStatus', true)
        await axios.post('/api/students', formFields)
        .then(response => {
            commit('newEntries', response.data);
            commit('newErrors', errors);
    ...
    async addEntryFamily({ commit }, formFields) {
        var errors = {};
        commit('newLoadingStatus', true)
        await axios.post('/api/students/family', formFields)
        .then(response => {
            commit('newEntries', response.data);
            commit('newErrors', errors);
        }).catch(error => {
    ...
    async addEntryAcademics({ commit }, formFields) {
        var errors = {};
        commit('newLoadingStatus', true)
        await axios.post('/api/students/academics', formFields)
        .then(response => {
            commit('newEntries', response.data);
            commit('newErrors', errors);
        }).catch(error => {
    ...
    async editStudentEntry({ commit }, payload) {
        var errors = {};
        commit('newLoadingStatus', true)
        await axios.put(`/api/students/${payload.id}`, payload.formFields)
        .then(response => {
    ...
    async editStudentFamilyEntry({ commit }, formFields) {
        var errors = {};
        commit('newLoadingStatus', true)
        await axios.put(`/api/students/${formFields.student_id}/family`, formFields)
        .then(response => {
            if(response.status == 200) {
                console.log(response)
                commit('setFlashMessage', response.data.message)
    ...
    async editStudentAcademics({ commit }, formFields) {
        var errors = {}
        commit('newLoadingStatus', true)
        await axios.put(`/api/students/academics/${formFields.academic_id}`, formFields)
        .then(response => {
            if(response.status == 200) {
                commit('setFlashMessage', response.data.message)
            }
    ...
    async addCourseMarks({ commit }, formFields) {
        var errors = {};
        commit('newLoadingStatus', true)
        await axios.post('/api/courses-marks', formFields)
        .then(response => {
            commit('newCourses', response.data);
            commit('newErrors', errors);
        }).catch(error => {
    ...
    async editSingleCourse({ commit }, payload) {
        var errors = {};
        commit('newLoadingStatus', true)
        await axios.put(`/api/courses/${payload.id}`, payload.formFields)
        .then(response => {
            console.log(response)
            commit('newSingleCourse', payload.formFields)
            commit('setFlashMessage', 'Course details updated successfully')
        }).catch(error => {
            if(error.response.status == 422) {
                for (const key in error.response.data.errors) {
                    errors[key] = error.response.data.errors[key][0];        
    ...
    async deleteSingleCourse({ commit }, id) {
        ...
        .then(response => {
            ...
            if(response.status == 204) {
                ...
                commit('setFlashMessage', 'The course has been deleted from the database.')
